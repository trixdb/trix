...
| 37 | ✅ **FIXED (2026-06-26)**: all 21 gitlinks are now registered in `.gitmodules` (was: only `trix-bots`), so `git submodule update --init --recursive` clones everything. `shannon` pinned to `branch = trix/main`; all repos now under `trixdb` (`trix-app`/`trix-sdk-go` migrated from `devghost`) | `.gitmodules` |
